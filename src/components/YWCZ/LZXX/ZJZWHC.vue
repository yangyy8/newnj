    ...
    <div class="yycontent">
          <el-table
           ...
           <el-table-column
             ...
             label="核查通过">
             <template slot-scope="scope">
             <a class="sb" @click="toLink (scope.row,'HCTG',scope.row.HCTG)" > {{scope.row.HCTG}}</a>
             </template>
           </el-table-column>
           <el-table-column
             prop="HCBTG"
             label="核查不通过">
             <template slot-scope="scope">
                <a class="sb" @click="toLink (scope.row,'HCBTG',scope.row.HCBTG)"> {{scope.row.HCBTG}}</a>
             </template>
           </el-table-column>
         </el-table>
    </div>
...
export default {
  ...
  methods: {
    ...
    download(){
      ...
      this.$api.post(this.Global.aport2+'/lzbjxx/exportLzzjhcData',p,
       r =>{
         this.downloadM(r)
       },e=>{},{},'blob')
    },
    ...
    getList(pd) {
      ...
      var url=this.Global.aport2+'/lzbjxx/getLzzjhcData';
      this.$api.post(url, p,
        r => {
          if(r.success){
    ...
    toLink(i,type,num){
        ...
        this.$router.push({name: 'ZWHCRYXX',query:{cdt:p}});
        // if(i.fj=='合计'){
        //     p.ssfjmc='';
