<template lang="html">
  <div class="">
    ...
    <el-col  :sm="24" :md="12" :lg="8"  class="input-item">
        ...
        <el-select v-model="pd.SSPCS" @change="getZrq(pd.SSPCS)" filterable clearable default-first-option placeholder="请选择"  size="small" class="input-input" :disabled="juState=='3'||juState=='4'" :no-data-text="pd.SSFJ==''||pd.SSFJ==undefined?'请先选择所属分局':'无数据'">
          <el-option
            v-for="item in PSC"
            :key="item.DM"
            :label="item.DM+' - '+item.MC"
